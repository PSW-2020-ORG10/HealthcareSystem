...
namespace EventStore.Service
{
    public class AppointmentSchedulingEventService : IAppointmentSchedulingEventService
    {
        ...
        public int GetHowManySchedulesInMinimumSteps()
        {
            if (GetSuccessfulAppointmentsAttempts().Count > 0)
            {
                return GetAll().Where(appointmentEvent => GetSuccessfulAppointmentsAttempts().Contains(appointmentEvent.Attempt)).ToList().GroupBy(appointmentEvent => appointmentEvent.Attempt).Where(grp => grp.Count() - 2 == 4).ToList().Count();
            }
            return 0;
        }
        ...
        public int GetHowManySchedulesInMediumSteps()
        {
            if (GetSuccessfulAppointmentsAttempts().Count > 0)
            {
                return GetAll().Where(appointmentEvent => GetSuccessfulAppointmentsAttempts().Contains(appointmentEvent.Attempt)).ToList().GroupBy(appointmentEvent => appointmentEvent.Attempt).Where(grp => grp.Count() - 2 > 4 && grp.Count() - 2 <= 7).ToList().Count();
            }
            return 0;
        }
        ...
        public int GetHowManySchedulesInMoreSteps()
        {
            if (GetSuccessfulAppointmentsAttempts().Count > 0)
            {
                return GetAll().Where(appointmentEvent => GetSuccessfulAppointmentsAttempts().Contains(appointmentEvent.Attempt)).ToList().GroupBy(appointmentEvent => appointmentEvent.Attempt).Where(grp => grp.Count() - 2 > 7).ToList().Count();
            }
            return 0;
        }
